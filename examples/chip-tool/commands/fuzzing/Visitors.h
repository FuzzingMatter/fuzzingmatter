...
template <typename T>
void ProcessDescriptorClusterResponse(std::shared_ptr<DecodedTLVElement> decoded, const app::ConcreteAttributePath & path,
                                      NodeId node);

extern template void ProcessDescriptorClusterResponse<EndpointId>(std::shared_ptr<DecodedTLVElement> decoded,
                                                                  const app::ConcreteAttributePath & path, NodeId node);
extern template void ProcessDescriptorClusterResponse<ClusterId>(std::shared_ptr<DecodedTLVElement> decoded,
                                                                 const app::ConcreteAttributePath & path, NodeId node);

void ProcessBasicInformationClusterResponse(std::shared_ptr<DecodedTLVElement> decoded, const app::ConcreteAttributePath & path,
                                            NodeId node);

} // namespace TLV

const types::AnyType & AttributeWrapperRead(AttributeWrapper * attribute);
...
bool IsEqual(const types::AnyType & lhs, const types::AnyType & rhs);
std::string AttributeValueAsString(const types::AnyType & attr);
std::string AttributeTypeAsString(const types::AnyType & attr);
